...
#define SD_OFF(sd) ((sd)->offset)
    ...
    {
        if (!data || !data->data || SD_OFF(data) + 0x158u > SD_SIZE(data))
            return -0x67;
        uint8_t *dst = SD_BUF(data) + SD_OFF(data);
    ...
    {
        if (!data || !data->data || SD_OFF(data) + 4u > SD_SIZE(data))
            return -0x67;
...
cacao::ProcessCtrlResult *
createResultWithCtx(imageprocessor::BypassCameraContext *ctx)
{

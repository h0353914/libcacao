...
namespace imageprocessor
{
    ...
    void SnapshotCallback::onHandleResult(const cacao::ProcessResultBase *result)
    {
        ...
        uint32_t reqId = 0;
        if (!imageprocessor::getSnapshotCtxAndId(result, &ctx, &reqId))
            return;
...
extern "C" void BypassCameraPhoto_finalize(JNIEnv *env, jobject /*thiz*/, imageprocessor::BypassCameraContext *ctx)
{
    ...
    {
        if (env)
            env->DeleteGlobalRef(ctx->photoJObj);
        ctx->photoJObj = nullptr;
    }

    delete ctx->snapshotReadyCb;
    ctx->snapshotReadyCb = nullptr;
    delete ctx->snapshotCb;
    ctx->snapshotCb = nullptr;
    delete ctx->burstPrepareCb;
    ctx->burstPrepareCb = nullptr;
    delete ctx->snapshotFreeCb;
    ctx->snapshotFreeCb = nullptr;
    delete ctx->burstFinishCb;
    ctx->burstFinishCb = nullptr;

    delete static_cast<cacao::ProcessCtrlResult *>(ctx->snapshotReadyResult);
    ctx->snapshotReadyResult = nullptr;
    delete static_cast<cacao::ProcessCtrlResult *>(ctx->burstPrepareResult);
    ctx->burstPrepareResult = nullptr;
    delete static_cast<cacao::ProcessCtrlResult *>(ctx->snapshotFreeResult);
    ctx->snapshotFreeResult = nullptr;
    delete static_cast<cacao::ProcessCtrlResult *>(ctx->burstFinishResult);
    ctx->burstFinishResult = nullptr;

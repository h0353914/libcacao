...
extern "C" void BypassCameraVideo_finalize(JNIEnv *env, jobject /*thiz*/, imageprocessor::BypassCameraContext *ctx)
{
    ...
    {
        if (env)
            env->DeleteGlobalRef(ctx->videoJObj);
        ctx->videoJObj = nullptr;
    }

    delete ctx->videoFinishCb;
    ctx->videoFinishCb = nullptr;
    delete ctx->videoPrepareCb;
    ctx->videoPrepareCb = nullptr;
    delete ctx->videoStartSuperSlowCb;
    ctx->videoStartSuperSlowCb = nullptr;
    delete ctx->videoStartRecCb;
    ctx->videoStartRecCb = nullptr;
    delete ctx->videoSuperSlowCb;
    ctx->videoSuperSlowCb = nullptr;

    delete static_cast<cacao::ProcessCtrlResult *>(ctx->videoFinishResult);
    ctx->videoFinishResult = nullptr;
    delete static_cast<cacao::ProcessCtrlResult *>(ctx->videoPrepareResult);
    ctx->videoPrepareResult = nullptr;
    delete static_cast<cacao::ProcessCtrlResult *>(ctx->videoStartSuperSlowResult);
    ctx->videoStartSuperSlowResult = nullptr;
    delete static_cast<cacao::ProcessCtrlResult *>(ctx->videoStartRecResult);
    ctx->videoStartRecResult = nullptr;
    delete static_cast<cacao::ProcessCtrlResult *>(ctx->videoSuperSlowResult);
    ctx->videoSuperSlowResult = nullptr;
}
